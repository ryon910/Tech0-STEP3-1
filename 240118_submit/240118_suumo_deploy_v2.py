...
if st.session_state['button1_clicked'] == True:
    ...
    for index, row in df_search_drop.iterrows():
        popup = folium.Popup(f"id：{row['id']}<br>名前：{row['name']}<br>家賃：{row['rent']}円<br>詳細URL：{row['detail_url']}<br>")
        folium.Marker(
    ...
    df_search.columns = ['id','物件名', '住所', '築年数', '構造', '階数', '賃料', '管理費', '敷金', '礼金', '間取り', '占有面積', '最寄駅路線1', '最寄駅1', '駅徒歩1', '最寄駅路線2', '最寄駅2', '駅徒歩2', '最寄駅路線3', '最寄駅3', '駅徒歩3', '緯度', '経度','物件画像URL','間取り画像URL','詳細情報URL' ]
    ...
    st.write('▼詳細情報：')
    id_input_str = st.text_input('物件のidを入力してください', value=0)
    if not id_input_str:
        st.write('待機中...')
    else:
        try:
            ...
            df_detail.columns = ['id','物件名', '住所', '築年数', '構造', '階数', '賃料', '管理費', '敷金', '礼金', '間取り', '占有面積', '最寄駅路線1', '最寄駅1', '駅徒歩1', '最寄駅路線2', '最寄駅2', '駅徒歩2', '最寄駅路線3', '最寄駅3', '駅徒歩3', '緯度', '経度','物件画像URL','間取り画像URL','詳細情報URL' ]
